...
> Schedule Request Dashboard — syncs from an internal Inventory API, persists in PostgreSQL, visualises VM→Baremetal placements.
...
└──────────────────────────┬──────────────────────────────────────────┘
                           │ SQL (drizzle-orm · node-postgres)
┌──────────────────────────▼──────────────────────────────────────────┐
│  Sync + DB layer — src/lib/sync.ts, src/db/                         │
│    PostgreSQL (DATABASE_URL) — schema.ts is the canonical shape     │
...
baremetal                                         ← topology + capacity snapshot
  ...
  max_vm_count, current_vm_count, ip_types (jsonb), snapshot_at
...
- **Full sync.** Each run fetches the complete Inventory snapshot and upserts it in one PostgreSQL transaction. Safer than an `updated_at` cursor until the Inventory API guarantees one.
...
- A reachable PostgreSQL — easiest is a local container:
  ```bash
  docker run -d --name schedview-pg \
    -e POSTGRES_USER=schedview \
    -e POSTGRES_PASSWORD=schedview \
    -e POSTGRES_DB=schedview \
    -p 5432:5432 \
    postgres:17
  ```
...
**First run**
```bash
export DATABASE_URL=postgres://schedview:schedview@localhost:5432/schedview
npm install
npm run db:generate  # produces SQL migration from src/db/schema.ts
npm run db:migrate   # applies the migration to DATABASE_URL
...
| `DATABASE_URL` | _required_ | Postgres connection string, e.g. `postgres://user:pass@localhost:5432/schedview` |
...
- **Drizzle over Prisma.** The schema is portable — moving between SQLite and Postgres only touches `src/db/schema.ts` and `src/db/client.ts`; the query layer is unchanged.
...
SchedView 把 scheduler 產出的調度結果（**Schedule Request → Schedule Placement**）視覺化成一個可查詢的儀表板。資料源是內部 **Inventory API**，瀏覽器無法直接存取；SchedView 在伺服器端扮演橋接角色，定期（以及按需）從 Inventory API 拉資料、寫入 PostgreSQL，並透過 Next.js React UI 呈現。
...
- **Sync + DB 層** — `src/lib/sync.ts` + `src/db/`：Drizzle ORM + node-postgres。5 分鐘週期由 `src/instrumentation.ts` 註冊，用 single-flight lock 避免重覆執行。
...
- **Full sync**：每次拉完整的 Inventory 快照、在一個 PostgreSQL 交易裡 upsert 全部資料。Inventory 尚未保證 `updated_at` 前，先採完整同步比較安全。
...
**前置條件**
- Node.js 20+（npm 隨附）
- 一個可連線的 PostgreSQL — 最簡單就在本機跑一個 container：
  ```bash
  docker run -d --name schedview-pg \
    -e POSTGRES_USER=schedview \
    -e POSTGRES_PASSWORD=schedview \
...
export DATABASE_URL=postgres://schedview:schedview@localhost:5432/schedview
npm install
npm run db:generate  # 從 src/db/schema.ts 產出 SQL migration
npm run db:migrate   # 套用到 DATABASE_URL 指向的 DB
...
| `DATABASE_URL` | _必填_ | Postgres 連線字串，例如 `postgres://user:pass@localhost:5432/schedview` |
...
- **Drizzle 而非 Prisma**：schema 是 portable 的——SQLite ↔ Postgres 之間切換只動 `src/db/schema.ts` 與 `src/db/client.ts`，查詢層完全不變。
